...
- 稳定池执行验证已支持展示最新 5m execution verification 摘要，并能区分原始 1h 研究 run 与派生 5m 执行验证 run
- 稳定池已新增基于 5m execution verification run 的过滤实验入口，生成 `run_type = execution_filter_experiment` 的派生 run
- 5m 执行过滤实验结果进入参数实验批次明细，但不覆盖稳定池最新执行验证结果，也不污染普通参数池 readmodel
- 已完成一次 BTC/USDT 5m execution run 最大回撤段归因分析，结论记录在 [2026-05-06 5m 执行验证与过滤研究交接](./2026-05-06-5m-execution-filter-research-handoff.md)
...
- 5m 执行过滤实验仍是研究工具，不改变主策略入场、止损、止盈或风险参数

## 8. 下一步

下一步优先做：

1. 基于最新 5m execution verification run 跑最小过滤实验，优先验证 `震荡程度 >= 0.8` 及其与 `20根局部位置 >= 0.8`、`连续顺向 1-2` 的少量组合
2. 把最大回撤归因候选转成显式 `signal_filter_sets`，区分“默认代理过滤”和“回撤归因候选过滤”
3. 增加过滤实验结果对比视图，直接比较源 5m execution run 与每个过滤 run 的收益、最大回撤、交易保留率和 OOS 变化
4. 完成 Research Note 决策状态机的前端筛选、展示和测试收口
5. 把 Research Note 的状态语义继续用于推荐卡和参数组表，沉淀“候选 / 观察 / 通过 / 拒绝 / 归档”的研究路径
6. 把总览和参数实验筛选继续下推到服务端查询参数，减轻大 workspace 下的前端过滤压力
7. 在研究闭环补齐后，再考虑更稳的本地异步执行器
8. 最后再推进 `StrategyRegistry` 或策略 v2，避免在研究语义未稳定前过早扩张策略空间
